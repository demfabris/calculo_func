...
for i, j in zip(DADOS_sujo, DADOS_limpo):
    if scan_escala(j) == 1 and coletar_dias(j) > 14:
        VET.update({str(coletar_nome(i)) + detect_troca(j): (coletar_dias(j), coletar_posto(i), 'R$'+str((coletar_dias(j)+5)*v)+',00')})
    else:
        VET.update({str(coletar_nome(i)) + detect_troca(j): (coletar_dias(j), coletar_posto(i), 'R$'+str(coletar_dias(j)*v)+',00')})
