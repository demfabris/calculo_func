...
for aux in STRING:
    aux1 = aux.replace(' ','')
    aux1 = aux1.replace('\n','')
    #aux1 = aux1.replace('TolerânciaDéb', '')
    #aux1 = aux1.replace('TolerânciaCréd', '')
    #aux1 = aux1.replace('Falta', '')
    #aux1 = aux1.replace('Extras', '')
    #aux1 = aux1.replace('InterJornada', '')
...
def detect_troca(t):                                                                                    ##detectar troca de horario no meio da folha
    ...
    for dia in vet:
        if len(dia) > 5:                                                                                            ##limpei strings dos dias, vet é uma lista de horarios com 7 horarios cada
                                                                                                                                ## encontrar maneira de calcular
...
def coletar_dias(STRING_limpa):
    aux = STRING_limpa.split('AssinaturadaChefia')
        ...
        #aux.pop()
        #aux=aux[::-1]
    #abono = aux[1].split('AssinaturadaChefia')
    #abono = abono[0]
